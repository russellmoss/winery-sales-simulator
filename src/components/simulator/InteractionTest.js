...
function InteractionTest() {
  ...
  const runTests = async () => {
    ...
    try {
      ...
      const validId = await addInteraction(
      ...
      setTestResults((prev) => [
        ...(prev || []),
        "Test 1: ✅ Valid interaction saved",
      ]);
      ...
      try {
        ...
        setTestResults((prev) => [
          ...(prev || []),
          "Test 2: ❌ Empty message was accepted",
        ]);
      } catch (err) {
        setTestResults((prev) => [
          ...(prev || []),
      ...
      try {
        ...
        setTestResults((prev) => [
          ...(prev || []),
          "Test 3: ❌ Missing role was accepted",
        ]);
      } catch (err) {
        setTestResults((prev) => [
          ...(prev || []),
          "Test 3: ✅ Missing role rejected",
        ]);
      }
      ...
      const hasOptimisticUpdate = interactions.some(
        (i) => i.message === testMessage
      );
      setTestResults((prev) => [
        ...(prev || []),
        ...
        <ul>
          {interactions.map((interaction, index) => (
            <li key={index}>
              ...
              {interaction.id && (
                <span style={{ color: "green" }}> (ID: {interaction.id})</span>
              )}
            </li>
          ))}
        </ul>
