...
const createSystemPrompt = (scenario) => {
  console.log(
    "Creating system prompt with scenario:",
...
${
  scenario.clientPersonality.preferences.interests.length > 0
    ? `- Interests: ${scenario.clientPersonality.preferences.interests.join(
        ", "
      )}`
    : ""
}
...
${scenario.behavioralInstructions.generalBehavior
  .map((behavior) => `- ${behavior}`)
  .join("\n")}

Tasting Behavior:
${scenario.behavioralInstructions.tastingBehavior
  .map((behavior) => `- ${behavior}`)
  .join("\n")}

Purchase Intentions:
${scenario.behavioralInstructions.purchaseIntentions
...
const createAssistantPrompt = (scenario) => {
  console.log(
    "Creating assistant prompt with scenario:",
    JSON.stringify(scenario, null, 2)
  );
...
export const sendMessageToClaude = async ({
  ...
  messages.forEach((msg, index) => {
    if (!msg.role || !msg.content) {
      throw new Error(
        `Message at index ${index} is missing required fields (role or content)`
      );
    }
  });
...
export const cleanupTranscription = async (transcription) => {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to cleanup transcription: ${
